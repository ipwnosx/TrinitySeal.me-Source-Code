...
<html>
    <head>
		<title>TrinitySeal - Edit Variable</title>
...
$varshit = mysqli_query($con, "SELECT * FROM `vars` WHERE `id` = '$client' AND `programtoken` = '$progtoken' AND `owner` = '$username'") or die(mysqli_error($con));
if(mysqli_num_rows($varshit) <= 0){
    echo "<div style=\"margin:auto;margin-top:10px;width:700px;\" class=\"ui negative message\">
                    <p>Attempting to edit invalid variable!</p>
                </div>";
        header("refresh:2; url=https://trinityseal.me/dashboard");
                die();
}
else{
    while($row = mysqli_fetch_array($varshit)){
        $varvalue = $row['value'];
        $varname = $row['name'];
      }
}
?>
        <div class="ui container">
            <div style="margin:auto; width:1000px; margin-bottom: 15px" class="ui inverted segment">
                <h2 class="ui inverted dividing header">Editing <?php echo xss_clean($varname); ?>...</h2>
                    <form class="ui form" method="POST" action="">
<?php
//updateuser
$newvarvalue = xss_clean(mysqli_real_escape_string($con, $_POST['value']));

if (isset($_POST['updatevar'])) {
    
if (!empty($newvarvalue)){ 
    $updatevarquery = mysqli_query($con, "UPDATE `vars` SET `value` = '$newvarvalue' WHERE `programtoken` = '$progtoken' AND `id` = '$client' AND `owner` = '$username'") or die(mysqli_error($con));
    if($updatevarquery){
        echo"<div style=\"margin:auto;margin-top:10px;width:970px;\" class=\"ui positive message\">
                    <p>Successfully updated variable!</p>
                </div><br>";
    ...
    echo"<div style=\"margin:auto;margin-top:10px;width:970px;\" class=\"ui negative message\">
                    <p>You must enter a variable value!</p>
                </div><br>";
}
}
?>

                    <div class="col-md-3"><strong><font color="white">Variable value:</font></strong></div>
                    <div class="ui action fluid input">
                        <input id="text" name="value" type="text" value="<?php echo xss_clean($varvalue); ?>">
                      </select>
                    </div>
                    <br>
                    <input id="submit" name="updatevar" type="submit" class="ui fluid red button" value="Update Variable">
                </form>
            </div>
